...
public:
    AnimatedText(std::string text, float duration) : m_text(text), m_duration(duration) {}
        ...
        while (window.isOpen())
        {
            ...
            while (elapsed.asSeconds()< m_duration) {
                for (int i = 0; i < m_text.length(); i++) {
                    str += getText()[i];
                    text.setString(str);
                    elapsed = sf::seconds(elapsed.asSeconds() + m_duration / words.length());
                    sf::sleep(elapsed);
